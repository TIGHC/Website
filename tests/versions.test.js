...
test("fetchVersion", async (t) => {
  ...
  await t.test("resolves null on a non-ok HTTP response", () => {
    return withMockFetch(
      ...
      () => fetchVersion("profiles").then((v) => assert.equal(v, null))
    );
  });
  ...
  await t.test("builds a request URL per repo key", () => {
    ...
    return withMockFetch(
      ...
      () =>
        Promise.all([fetchVersion("engine"), fetchVersion("profiles"), fetchVersion("website")]).then(
          () => {
            assert.equal(requested.length, 3);
            assert.ok(requested[0].includes(REPOS.engine));
            assert.ok(requested[1].includes(REPOS.profiles));
            assert.ok(requested[2].includes(REPOS.website));
          }
        )
    );
  });
